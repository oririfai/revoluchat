defmodule RevoluchatWeb.AdminDashboardLive.ApiKeysSection do
    ...
    <div class="max-w-4xl space-y-8 pb-20">
      <.card label="Generate New API Key">
        <form phx-submit="create_api_key" class="flex gap-4 items-end">
          <div class="flex-1">
            <label class="block text-sm font-medium text-gray-700 mb-1">Developer / App Name</label>
            <input
              type="text"
              name="name"
              placeholder="e.g. Android App, Web SDK, Jhon Doe"
              required
      ...
      <%= if @show_delete_modal do %>
        ...
        >
          Are you sure you want to permanently delete this API Key? This action cannot be undone and any application using this key will lose access immediately.
          <:footer>
            <.revolu_button phx-click="confirm_delete_api_key" variant="solid" class="bg-red-600 hover:bg-red-700">
              Confirm Delete
            </.revolu_button>
            <.revolu_button phx-click={hide_dashboard_modal("delete-key-modal") |> JS.push("close_modal")} variant="white">
              Cancel
            </.revolu_button>
          </:footer>
        </.modal>
      ...
      <%= if @show_revoke_modal do %>
        <.modal
          id="revoke-key-modal"
          show={@show_revoke_modal}
          type="warning"
          title="Revoke API Key"
          on_cancel={JS.push("close_modal")}
        >
          Are you sure you want to revoke this API Key? The key will remain in the system but will no longer be valid for authentication. You can still delete it permanently later.
          <:footer>
            <.revolu_button phx-click="confirm_revoke_api_key" variant="solid" class="bg-amber-600 hover:bg-amber-700">
              Confirm Revoke
            </.revolu_button>
            <.revolu_button phx-click={hide_dashboard_modal("revoke-key-modal") |> JS.push("close_modal")} variant="white">
              Cancel
            </.revolu_button>
          </:footer>
        </.modal>
